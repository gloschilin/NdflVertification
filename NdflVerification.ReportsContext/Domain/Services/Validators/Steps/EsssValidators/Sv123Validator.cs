...
namespace NdflVerification.ReportsContext.Domain.Services.Validators.Steps.EsssValidators
{
    public class Sv123Validator : BaseReportStepValidator<Файл>
    {
        ...
        {
            if (entity.Документ.РасчетСВ.ОбязПлатСВ.РасчСВ_ОССВНМ.УплСВПрев.УплВсегоПосл3М.Признак.ToInt() == 1
                && entity.Документ.РасчетСВ.ОбязПлатСВ.УплПревОСС.УплПерОСС.СумСВУпл1М
                != entity.Документ.РасчетСВ.ОбязПлатСВ.РасчСВ_ОССВНМ.УплСВПрев.УплВсегоПосл3М.Сумма)
            {
                return false;
            }
